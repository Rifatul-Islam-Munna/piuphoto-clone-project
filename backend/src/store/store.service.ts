...
import {
  ...
  StoreOrderDocument,
  StoreOrderStatus,
} from './entities/store-order.entity';
import {
  StoreSettings,
...
export class StoreService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly settings: Model<StoreSettingsDocument>,
    @InjectModel(StoreOrder.name)
    private readonly orders: Model<StoreOrderDocument>,
  ...
  onModuleInit() {
    ...
    const reconcileMs =
      Math.max(
        Number(this.config.get<string>('STRIPE_RECONCILE_SECONDS')) || 60,
        15,
      ) * 1000;
    this.paymentWorker = setInterval(
  ...
  private async flushPendingDeliveries() {
    if (!this.notifier.emailConfigured() && !this.notifier.whatsappConfigured())
      return;
    try {
      const rows = await this.orders
        .find({
          ...
          downloadTokenCipher: { $exists: true },
          $or: [
            { deliveryAttempts: { $lt: 5 } },
            { deliveryAttempts: { $exists: false } },
          ],
        })
        .select('+downloadTokenCipher')
        .sort({ paidAt: 1 })
        .limit(20)
        .exec();
      for (const order of rows) {
        if (order.downloadTokenCipher)
          await this.sendDelivery(
            order,
            this.decrypt(order.downloadTokenCipher),
          );
      }
    } catch (error) {
  ...
  private async reconcileStripePayments() {
    try {
      const pending = await this.orders
        ...
        .sort({ lastPaymentCheckAt: 1, createdAt: 1 })
        .limit(20)
        .exec();
      for (const order of pending) {
        const key = await this.stripeKeyForEvent(String(order.eventId));
        const session = await this.stripeGet<any>(
          `checkout/sessions/${encodeURIComponent(String(order.stripeSessionId))}`,
          key,
        );
        await this.orders.updateOne(
          { _id: order._id },
          { $set: { lastPaymentCheckAt: new Date() } },
        );
        if (session.payment_status === 'paid') {
          await this.ensurePaid(
            order,
          ...
          await this.orders.updateOne(
            { _id: order._id, status: StoreOrderStatus.PENDING },
            { $set: { status: StoreOrderStatus.FAILED } },
          );
        }
      }
      ...
      const paid = await this.orders
        ...
        .sort({ lastPaymentCheckAt: 1, paidAt: 1 })
        .limit(20)
        .exec();
      for (const order of paid) {
        const key = await this.stripeKeyForEvent(String(order.eventId));
        const intent = await this.stripeGet<any>(
          `payment_intents/${encodeURIComponent(String(order.paymentIntentId))}`,
          key,
        ...
        if (refunded) {
          await this.orders.updateOne(
            { _id: order._id, status: StoreOrderStatus.PAID },
            {
              $set: {
  ...
  private platformStripeKey() {
    const value = this.config.get<string>('STRIPE_SECRET_KEY')?.trim();
    if (!value) throw new HttpException('Stripe secret key missing', 500);
    return value;
  }
  private stripeKeyFor(config: any) {
  ...
  private stripeWebhookSecretFor(config: any) {
    if (config?.useCustomStripe === true && config?.stripeWebhookSecretCipher)
      return this.decrypt(String(config.stripeWebhookSecretCipher));
  ...
  private tokenSecret() {
    return createHash('sha256')
      .update(
        this.config.get<string>('STORE_TOKEN_SECRET') ||
          this.config.get<string>('ACCESS_TOKEN') ||
          'change-store-token-secret',
      )
      .digest();
  }
  ...
  private decrypt(value: string) {
    ...
    const d = createDecipheriv('aes-256-gcm', this.tokenSecret(), iv);
    d.setAuthTag(tag);
  ...
  private async getSettings(eventId: string, withSecrets = false) {
    ...
    if (withSecrets)
      query.select('+stripeSecretCipher +stripeWebhookSecretCipher');
    return query.lean();
  }
  ...
  async publicCatalog(eventId: string, albumId?: string) {
    ...
    const event = await this.events
      .findOne({ _id: this.oid(eventId), isActive: true, isPublished: true })
      .select('title description branding')
      .lean();
    if (!event || !config?.enabled)
      throw new HttpException('Store is not available', 404);
    const allowedAlbums = (config.saleAlbumIds || []).map(String);
    if (albumId && allowedAlbums.length && !allowedAlbums.includes(albumId))
      throw new HttpException('This category is not for sale', 404);
    const rows = await this.images
      ...
      .select('_id albumId createdAt isEnhanced')
      .sort({ createdAt: -1 })
      .limit(1000)
      .lean();
    return {
      ...
      data: rows.map((row) => ({
        _id: String(row._id),
        albumId: row.albumId ? String(row.albumId) : undefined,
        isEnhanced: row.isEnhanced,
        previewUrl: `/store/public/preview?eventId=${eventId}&imageId=${row._id}`,
      })),
  ...
  async preview(eventId: string, imageId: string) {
    const config = await this.getSettings(eventId);
    if (!config?.enabled) throw new HttpException('Store unavailable', 404);
    const image = await this.images
      .findOne({
        _id: this.oid(imageId),
        eventId: this.oid(eventId),
        isPublished: { $ne: false },
        isForSale: { $ne: false },
        mediaType: { $ne: 'video' },
      })
      .select('imageUrl albumId')
      .lean();
    if (!image) throw new HttpException('Photo not found', 404);
    const allowedAlbums = (config.saleAlbumIds || []).map(String);
    if (
      allowedAlbums.length &&
      (!image.albumId || !allowedAlbums.includes(String(image.albumId)))
    )
      throw new HttpException('Photo is not available for sale', 404);
    const event = await this.events.findById(eventId).select('branding').lean();
    ...
    const quality = Math.min(
      Math.max(Number(config.previewQuality) || 64, 30),
      90,
    );
    const maxWidth = Math.min(
      Math.max(Number(config.previewMaxWidth) || 1200, 400),
      2400,
    );
    ...
    if (config.watermarkedPreview !== false) {
      const branding: any = event?.branding || {};
      const opacity = Math.min(
        Math.max(Number(branding.watermarkOpacity) || 0.7, 0.05),
        1,
      );
      const scale = Math.min(
        Math.max(Number(branding.watermarkScale) || 24, 5),
        80,
      );
      const position = String(branding.watermarkPosition || 'bottom_right');
      ...
      if (branding.watermarkUrl) {
        const wmResponse = await axios.get<ArrayBuffer>(branding.watermarkUrl, {
          responseType: 'arraybuffer',
          timeout: 15000,
        ...
        const targetWidth = Math.max(
          40,
          Math.round(resized.info.width * (scale / 100)),
        );
        const wm = await sharp(Buffer.from(wmResponse.data))
          .resize({ width: targetWidth, withoutEnlargement: true })
          .png()
          .toBuffer({ resolveWithObject: true });
        const encoded = wm.data.toString('base64');
        const svg = Buffer.from(
          `<svg width="${wm.info.width}" height="${wm.info.height}"><image href="data:image/png;base64,${encoded}" width="100%" height="100%" opacity="${opacity}"/></svg>`,
        );
        ...
        const svg = Buffer.from(
          `<svg width="600" height="120"><text x="300" y="72" text-anchor="middle" font-size="48" font-family="Arial" fill="white" fill-opacity="${opacity}">PREVIEW</text></svg>`,
        );
        pipeline = pipeline.composite([{ input: svg, gravity: 'center' }]);
      }
    }
  ...
  private price(
    config: StoreSettingsDocument | Record<string, any>,
    count: number,
  ) {
    if (config.bundlePrice > 0 && count >= config.bundleMinPhotos)
      return Number(config.bundlePrice);
  ...
  async checkout(dto: StoreCheckoutDto) {
    ...
    if (checkoutKey) {
      try {
        order = (await this.orders
          ...
          .exec()) as StoreOrderDocument;
      } catch (error: any) {
        if (error?.code !== 11000) throw error;
        order = (await this.orders
          .findOne({ checkoutIdempotencyKey: checkoutKey })
          .exec()) as StoreOrderDocument;
      }
      ...
      if (order.status === StoreOrderStatus.PAID && order.stripeSessionId) {
        return {
          ...
          url: `${this.frontendUrl()}/#/store/order/${order._id}?session_id=${encodeURIComponent(order.stripeSessionId)}`,
          amount: order.amount,
      ...
      if (order.status === StoreOrderStatus.FAILED) {
        order.status = StoreOrderStatus.PENDING;
        await order.save();
      }
    } else {
    ...
    params.append(
      'success_url',
      `${this.frontendUrl()}/#/store/order/${order._id}?session_id={CHECKOUT_SESSION_ID}`,
    );
    params.append(
      'cancel_url',
      `${this.frontendUrl()}/#/store/${dto.eventId}?checkout=cancel`,
    );
    ...
    params.append('metadata[eventId]', dto.eventId);
    params.append(
      'line_items[0][price_data][currency]',
      String(config.currency || 'USD').toLowerCase(),
    );
    params.append(
    ...
    try {
      ...
      if (checkoutKey)
        headers['Idempotency-Key'] = `airpix-store-${checkoutKey}`;
      ...
      order.stripeSessionId = data.id;
      order.stripeCheckoutUrl = data.url;
      order.status = StoreOrderStatus.PENDING;
      await order.save();
      return {
        orderId: String(order._id),
  ...
  private verifyStripeSignature(
    ...
    if (!rawBody || !signatureHeader)
      throw new ForbiddenException('Invalid Stripe webhook');
    ...
    if (Math.abs(Math.floor(Date.now() / 1000) - Number(timestamp)) > 300)
      throw new ForbiddenException('Expired Stripe webhook');
  ...
  private async ensurePaid(
    order: StoreOrderDocument | any,
    paymentIntentId?: string,
  ) {
    let full = await this.orders
      .findById(order._id)
      .select('+downloadTokenCipher')
      .exec();
    if (!full) throw new HttpException('Order not found', 404);
    if (full.status === StoreOrderStatus.REFUNDED)
      throw new HttpException('Order refunded', 400);
    if (full.status !== StoreOrderStatus.PAID || !full.downloadTokenCipher) {
      ...
      const set: Record<string, unknown> = {
        ...
        downloadTokenHash: this.hash(tokenCandidate),
        downloadTokenCipher: this.encrypt(tokenCandidate),
        downloadExpiresAt: new Date(
          Date.now() + (config?.downloadExpiresHours || 72) * 3600000,
        ),
      };
      ...
      const claimed = await this.orders
        .findOneAndUpdate(
          {
            _id: full._id,
            status: { $ne: StoreOrderStatus.REFUNDED },
            $or: [
              { status: { $ne: StoreOrderStatus.PAID } },
              { downloadTokenCipher: { $exists: false } },
              { downloadTokenCipher: null },
            ],
          },
          { $set: set },
          { new: true },
      ...
      await this.orders.updateOne(
        { _id: full._id, status: StoreOrderStatus.PAID },
        { $set: { paymentIntentId } },
      );
      full.paymentIntentId = paymentIntentId;
    }
    if (!full.downloadTokenCipher)
      throw new HttpException('Order delivery token is unavailable', 500);
  ...
  private async sendDelivery(order: StoreOrderDocument, token: string) {
      ...
      .findOneAndUpdate(
        {
          ...
          deliverySentAt: { $exists: false },
          $or: [
            { deliverySendingAt: { $exists: false } },
            { deliverySendingAt: null },
            { deliverySendingAt: { $lt: stale } },
          ],
        },
        {
          $set: { deliverySendingAt: new Date() },
    ...
    try {
      ...
      await this.orders.updateOne(
        { _id: order._id, deliverySentAt: { $exists: false } },
        {
          $set: { deliverySentAt: new Date() },
          $unset: { deliverySendingAt: '', deliveryError: '' },
        },
      );
    } catch (error) {
      this.logger.warn(
        `store-delivery-failed order=${order._id} ${String(error)}`,
      );
      await this.orders.updateOne(
        { _id: order._id },
        {
  ...
  async webhook(
    ...
    const config = eventId ? await this.getSettings(eventId, true) : undefined;
    const secret = eventId
      ? this.stripeWebhookSecretFor(config)
      : this.config.get<string>('STRIPE_WEBHOOK_SECRET')?.trim() || '';
    const verified = this.verifyStripeSignature(rawBody, signature, secret);
    if (!verified) {
      return {
        received: true,
        verified: false,
    ...
    const event = body || JSON.parse(rawBody!.toString('utf8'));
    const object = event?.data?.object || {};
    const eventFilter = eventId ? { eventId: this.oid(eventId) } : {};
    if (
      [
      ...
      const order = await this.orders
        .findOne({
          ...
          $or: [
            ...
            {
              _id: Types.ObjectId.isValid(object?.metadata?.orderId || '')
                ? this.oid(object.metadata.orderId)
                : undefined,
            },
          ].filter((x: any) => Object.values(x)[0] !== undefined),
        })
        .exec();
      if (
        order &&
        (object.payment_status === 'paid' || event.type.endsWith('succeeded'))
      )
        await this.ensurePaid(
          order,
    ...
    if (event.type === 'charge.refunded' && object.payment_intent) {
      await this.orders.updateMany(
        { ...eventFilter, paymentIntentId: object.payment_intent },
        {
          $set: {
            status: StoreOrderStatus.REFUNDED,
            refundedAt: new Date(),
  ...
  async verifyCheckout(dto: StoreVerifyDto) {
    const order = await this.orders
      .findOne({ _id: this.oid(dto.orderId), stripeSessionId: dto.sessionId })
      .exec();
    if (!order) throw new HttpException('Order not found', 404);
    const key = await this.stripeKeyForEvent(String(order.eventId));
    const { data } = await axios.get<any>(
      `https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(dto.sessionId)}`,
      { headers: { Authorization: `Bearer ${key}` }, timeout: 15000 },
    );
    if (data.payment_status !== 'paid')
      throw new HttpException('Payment is not completed yet', 400);
    const paid = await this.ensurePaid(
      order,
    ...
    return {
      ...
      orderId: String(order._id),
      orderNo: order.orderNo,
      downloadToken: paid.token,
      expiresAt: paid.order.downloadExpiresAt,
    };
  }

  async orderByToken(orderId: string, token: string) {
    const order: any = await this.orders
      .findOne({
        _id: this.oid(orderId),
        downloadTokenHash: this.hash(token),
        status: StoreOrderStatus.PAID,
        downloadExpiresAt: { $gt: new Date() },
      })
      .populate('eventId', 'title')
      .populate('imageIds', '_id createdAt')
      .lean();
    if (!order)
      throw new ForbiddenException('Order download link is invalid or expired');
  ...
  async downloadPurchased(orderId: string, token: string, imageId: string) {
    const order = await this.orders
      .findOne({
        ...
        status: StoreOrderStatus.PAID,
        downloadExpiresAt: { $gt: new Date() },
        imageIds: this.oid(imageId),
      })
      .select('eventId')
      .lean();
    if (!order)
      throw new ForbiddenException(
        'Purchased file access is invalid or expired',
      );
    const image = await this.images
  ...
  async updateSettings(dto: StoreSettingsDto, userId?: string, role?: string) {
    await this.members.assertCanManage(dto.eventId, userId, role);
    const current: any = await this.getSettings(dto.eventId, true);
    ...
    await this.settings
      .findOneAndUpdate(
        { eventId: this.oid(dto.eventId) },
        { $set: set, $setOnInsert: { eventId: this.oid(dto.eventId) } },
        { upsert: true, new: true, setDefaultsOnInsert: true },
      )
      .lean();
    const result = await this.settingsForPlanner(dto.eventId, userId, role);
    return { message: 'Store settings updated', data: result.data };
  }
  ...
  async updatePhotoSale(dto: StoreSaleDto, userId?: string, role?: string) {
    await this.members.assertCanManage(dto.eventId, userId, role);
    const ids = [...new Set(dto.imageIds)].map((id) => this.oid(id));
    const result = await this.images.updateMany(
      { _id: { $in: ids }, eventId: this.oid(dto.eventId) },
      { $set: { isForSale: dto.isForSale } },
    );
    return {
      message: dto.isForSale
        ? 'Photos enabled for sale'
        : 'Photos removed from sale',
      updated: result.modifiedCount,
    };
  }
  async listOrders(query: StoreOrderQueryDto, userId?: string, role?: string) {
    await this.members.assertCanManage(query.eventId, userId, role);
    const filter: any = { eventId: this.oid(query.eventId) };
